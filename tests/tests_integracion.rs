use tp_individual::ajedrez::{
    comenzar_juego, inicializar_piezas, jugar_ajedrez, model::resultado::Resultado,
};
...
fn test_ajedrez_dimension_invalida() {
    assert_eq!(
        inicializar_piezas(&String::from("dimension_invalida.txt")).unwrap_err(),
        "Error: [Dimensión del tablero errónea]"
    );
}
